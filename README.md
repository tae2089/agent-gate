...
1. **skill-invocation-verifier** — `Stop` hook. 결과물만으로 대체할 수 없는 독립 의미 판정 절차가 누락되면 턴 종료를 차단. 기본 정책은 `artifact-judge` 두 규칙뿐이며 일반적인 skill 선택은 모델에 맡긴다. ✅ 구현됨
...
5. **scenario-gate** — AC/P에서 만든 관찰 가능한 Scenario Contract의 원자 observation을 구현·검증 코드 위치와 최신 실행에 연결하고 Scenario Evidence Coverage를 계산한다. 자식으로 분해해도 직접 Full 부모의 완료 경계를 유지하며, 필수 observation 100%와 모든 check 통과를 Stop/CI에서 강제한다. ✅ 구현됨
6. **stage1-autorun** — 미도입. 편집마다 lint/test를 실행하면 작은 child 작업 수에 비례해 비용과 중복 실행이 늘어난다. 실행 증거는 편집마다가 아니라 작업 완료·CI 경계의 scenario run/completion이 소유한다.
...
8. **scope-gate** — 미도입. Child의 편집 파일 범위를 별도 manifest와 hook 정책으로 강제하는 비용이 현재의 실수 감소 효과보다 크다. 부모 Full의 P/AC 상속과 scenario 완료 검증만 유지한다.
...
- Full root의 `implementation.md`는 structural lint가 번호형 수도코드와 제어 흐름 Mermaid 블록을 필수 검사한다. 특정 skill 호출 여부가 아니라 산출물 결과를 판정한다.
...
Full 부모는 `scenario-design`으로 `_workspace/<task>/scenario-contract.json`을 만든다. 각 Then은 고유한 `O-*` observation ID와 관찰 가능한 expectation을 가지며, 각 scenario/check는 다른 scenario와 공유하지 않는 runner 하나를 사용한다. 자식은 별도 contract/overlay/evidence/result를 만들지 않고 직접 Full 부모의 전체 완료 경계를 사용한다. 자식에서 빠진 행동을 발견하면 부모 contract에 먼저 추가한다.

```bash
# 구현 전 contract 구조와 AC/P coverage 확인
python3 scripts/scenario_gate.py readiness _workspace/<full-task> --project-root .

# 구현 후 독립 evidence 판정용 current template
python3 scripts/scenario_gate.py evidence-template \
  _workspace/<full-task> --project-root . > /tmp/scenario-evidence-template.json
# artifact-judge 결과를 Full 부모의 scenario-evidence.json으로 기록

# 저장소가 선택한 언어/프레임워크 check 실행 및 부모 결과 원자 기록
python3 scripts/scenario_gate.py run _workspace/<active-task> --project-root . --json

# Stop hook/CI와 동일한 완료 판정
python3 scripts/scenario_gate.py completion _workspace/<active-task> --project-root . --json
```

설정 파일이 없으면 scenario gate는 비활성화되고, 있으면 단일 fail-closed 정책으로 동작한다. Rollout mode, risk/level, child overlay/ownership/parent-candidate는 없다.

Scenario Evidence Coverage는 각 필수 observation에 구현 위치, 검증 위치, 현재 source에서 통과한 소유 scenario가 모두 있는 비율이다. Python은 exact ID, 안전한 project-relative path/line, task/flow/contract/source hash와 실행 결과를 재검증한다. 독립 판정자는 observation과 코드 위치의 의미 관계만 검토하며 runner 설정이나 명령은 보지 않는다. 필수 observation은 가중하지 않고 완료 기준은 100%다. 이 값은 코드의 절대 정확도가 아니라 작성된 시나리오에 대한 추적 증거 완성도다.

Runner는 shell 없이 argv를 실행하고 exit code만 해당 scenario 하나에 기록한다. 상속 환경은 PATH·locale·temp·주요 언어 toolchain 경로로 제한하며 timeout, 출력 상한, process-tree 종료와 원자적 결과 저장을 유지한다. 결과는 contract, exact runner config, Git source fingerprint에 결합된다. Runner 설정은 저장소의 신뢰된 설정이며 잘못된 명령의 의미를 LLM review로 보완하지 않는다. OS 수준 network sandbox는 제공하지 않으므로 production credential을 주입하지 않는다.

토큰 비용은 task/flow/contract/source가 바뀌어 evidence mapping을 다시 판정할 때만 발생한다. Runner 설정만 바뀌면 기존 evidence는 유지하고 실행 결과만 다시 만든다. Hook, 점수 계산, runner 실행은 LLM을 호출하지 않는다.
