...
2. **stage1-autorun** — `PostToolUse(Write|Edit)` hook. 편집마다 lint/test 자동 실행 — 기계 검증이 에이전트 선의에 의존하지 않게.
3. **scope-gate** — `PreToolUse` hook. dispatch packet의 `allowed_scope`를 읽어 범위 밖 편집 차단.
