...
| Loop Pack (skills/ + scripts/) | bounded 피드백 루프의 입력·phase·종료 정책 | `evolution-loop`, `ci-repair-loop`, `assurance-loop`, `research-adoption-loop` |
...
- **assurance-loop** — 사용자가 지정한 diff를 `Inspect → Review → Address → Verify`로 반복하고 actionable finding이 없으며 Completion이 fresh 100%일 때 `review-clean`으로 종료합니다.
...
`assurance-loop`는 사용자가 review와 수정 모두를 요청한 정확한 diff, branch,
