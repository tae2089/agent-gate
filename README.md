...
다음 lifecycle 지원 기능도 번들되며 현재 plugin manifest에는 **default-enabled** 상태입니다. 두 Gate의 판정에는 참여하지 않습니다.

- **skill-invocation-verifier** — 명시적 artifact 평가 요청에 필요한 `artifact-judge` 호출을 Stop에서 확인합니다.
- **context-watermark / handoff-reinject** — 컨텍스트 임계점의 handoff 작성과 compaction 이후 재주입을 담당합니다.
