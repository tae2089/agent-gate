...
Runner는 범용 exit-code만 지원하고 shell 없이 실행한다. `critical` scenario는 runner를 독점해야 하며, standard runner가 5개를 넘는 scenario를 맡으면 흐름 분리 검토 경고가 발생한다. 같은 runner의 scenario들은 명령 성공 시 모두 통과하므로 독립 검토는 command가 할당된 scenario를 모두 선택하고 테스트가 없을 때 실패하는지 확인하며, 이 판단은 exact runner config hash에 결합된다. 상속 환경은 PATH·locale·temp·주요 언어 toolchain 경로로 제한한다. 결과는 effective scenario canonical hash, exact runner config hash, Git HEAD/경로 제한 tracked diff, untracked 파일의 내용이 아닌 path/mode/size/mtime metadata에 결합한다. 따라서 로컬 freshness는 우발적 stale 방지이고, CI의 clean commit identity가 가장 강한 증거다. OS 수준 network sandbox는 제공하지 않으므로 runner 설정은 신뢰된 저장소에서만 사용하고 production credential을 주입하지 않는다.
