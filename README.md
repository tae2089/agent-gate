...
2. **context-watermark** — `Stop` hook에서 transcript 사용률이 임계(기본 80%)를 넘으면 handoff 작성을 요구한다. Codex는 `PreCompact(manual|auto)`에서도 usage와 무관하게 유효한 현재-turn handoff를 최종 확인한다. Claude와 Codex의 서로 다른 block 응답 계약을 각각 사용한다. ✅ 구현됨
...
5. **scenario-gate** — AC/P에서 만든 관찰 가능한 Scenario Contract의 각 scenario를 전용 executable check에 연결하고 최신 실행의 추적 완성도를 계산한다. 자식으로 분해해도 직접 Full 부모의 완료 경계를 유지하며, 모든 필수 scenario의 최신 성공을 Stop/CI에서 강제한다. ✅ 구현됨
...
Full 부모는 `scenario-design`으로 `_workspace/<task>/scenario-contract.json`을 만든다. 각 Then은 관찰 가능한 결과를 평문 문자열로 선언하며, 각 scenario/check는 다른 scenario와 공유하지 않는 runner 하나를 사용한다. 자식은 별도 contract/overlay/result를 만들지 않고 직접 Full 부모의 전체 완료 경계를 사용한다. 자식에서 빠진 행동을 발견하면 부모 contract에 먼저 추가한다.
...
시나리오 추적 완성도는 최신 실행에서 통과한 필수 scenario 수를 전체 필수 scenario 수로 나눈 값이다. Python은 exact scenario ID와 task/flow/contract/source/runner-config freshness를 재검증하며 완료 기준은 100%다. 이 값은 선언된 시나리오 실행 상태이지, 시나리오 자체의 완전성이나 전체 코드 품질 점수가 아니다.

Runner는 shell 없이 argv를 실행하고 exit code만 해당 scenario 하나에 기록한다. 상속 환경은 PATH·locale·temp·주요 언어 toolchain 경로로 제한하며 timeout, 출력 상한, process-tree 종료와 원자적 결과 저장을 유지한다. 결과는 task, flow, contract, exact runner config, Git source fingerprint에 결합된다. Runner 설정은 저장소의 신뢰된 완료 권위이며 잘못된 테스트 의미를 LLM 판정으로 보완하지 않는다. OS 수준 network sandbox는 제공하지 않으므로 production credential을 주입하지 않는다.

Scenario run, completion, hook은 LLM을 호출하지 않아 토큰 비용이 없다. 시나리오 검토는 authoring 보조로 선택할 수 있지만 완료 판정에는 참여하지 않는다.
...
를 Stop hook에 추가. 옵션: `--window 200000` (컨텍스트 윈도우), `--threshold 0.8` (차단 임계). Codex 설정은 같은 명령을 `PreCompact`의 `manual|auto`에도 연결한다.
...
- 차단 출력은 Claude의 `decision/reason`과 Codex의 `continue/stopReason/systemMessage` 계약을 host 입력에 맞춰 선택한다.
