...
4. **readiness-gate** — `task.md` 모호함과 `implementation.md` 준비도를 독립 판정하고, 문서 해시·근거 인용·AC 추적성을 코드가 재검증. 유효한 assessment에 session이 바인딩되기 전에는 명시적 문서 예외 외의 직접 프로젝트 편집을 차단. Claude/Codex 설정 포함. ✅ 구현됨
...
- 유효한 assessment를 쓴 session만 직접 프로젝트 `Write`/`Edit`/`apply_patch` 가능; 확장자 없는 빌드 파일과 미등록 확장자도 기본 보호되며 문서 변경 즉시 stale 처리
