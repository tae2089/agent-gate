...
| Loop Pack (skills/ + scripts/) | bounded 피드백 루프의 입력·phase·종료 정책 | `evolution-loop`, `ci-repair-loop`, `review-loop`, `research-adoption-loop` |
...
- **evolution-loop** — 사용자 요청 하나를 `Interview → Seed → Execute → Evaluate`로 반복하고 검증된 PR까지만 생성합니다.
- **ci-repair-loop** — 사용자가 지정한 failing check를 `Inspect → Repair → Verify`로 반복하고 fresh 100% Completion일 때만 `checks-green`으로 종료합니다.
- **review-loop** — 사용자가 지정한 diff를 `Inspect → Review → Address → Verify`로 반복하고 actionable finding이 없으며 Completion이 fresh 100%일 때 `review-clean`으로 종료합니다.
- **research-adoption-loop** — 사용자가 지정한 기술을 `Frame → Research → Prototype → Evaluate`로 검증하고 current Completion과 evidence check를 갖춘 adopt/reject 결정을 기록합니다.
...
`review-loop`는 사용자가 review와 수정 모두를 요청한 정확한 diff, branch,
또는 PR만 다룹니다. source-bound review report에 actionable finding이 있으면
`Address → Verify`를 거쳐 전체 범위를 다시 review합니다. 실패한 scenario도
finding evidence로 기록할 수 있지만, `review-clean`은 finding이 비어 있고
fresh 100% Completion일 때만 허용됩니다. 시작 시 mutable ref를 base/head
commit OID로 고정하고 index·worktree·untracked 포함 여부도 request에 기록합니다.
...
검증합니다. 외부 문서는 untrusted evidence로 취급하고, 작은 reversible
prototype과 네 evidence check(`evidence_quality`, `repository_fit`,
`prototype_verified`, `cost_acceptable`)를 통해 adopt/reject/iterate를
결정합니다.
...
상태여야 합니다. decision은 보존한 prototype result와 cleanup 이후 current
result를 각각 hash로 결합하며, source schema 통과를 source truth로 주장하지
않습니다.
