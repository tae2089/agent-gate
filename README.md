...
4. **readiness-gate** — `task.md` 모호함과 `implementation.md` 준비도를 독립 판정하고, 문서 해시·근거 인용·AC 추적성을 코드가 재검증. 유효한 assessment에 session이 바인딩되기 전에는 직접 source 편집을 차단. Claude/Codex 설정 포함. ✅ 구현됨
5. **stage1-autorun** — `PostToolUse(Write|Edit)` hook. 편집마다 lint/test 자동 실행 — 기계 검증이 에이전트 선의에 의존하지 않게.
6. **artifact-lint** — 산출물 구조 lint (1층/$0). 가중합 점수 + 차원별 하한선(floor), handoff/task/implementation 타입. watermark와 연동돼 "빈껍데기 handoff"로는 block을 통과 못 함. 2층(LLM rubric)은 [docs/rubric-judge.md](docs/rubric-judge.md) 템플릿. ✅ 구현됨
7. **scope-gate** — `PreToolUse` hook. dispatch packet의 `allowed_scope`를 읽어 범위 밖 편집 차단.
...
- 유효한 assessment를 쓴 session만 직접 source-code `Write`/`Edit`/`apply_patch` 가능; 문서 변경 즉시 stale 처리
- 잘못된 assessment를 쓰면 PostToolUse가 validator의 첫 진단들을 즉시 반환하며 session을 바인딩하지 않음
- Claude의 canonical `artifact-judge`를 `.agents/skills` 상대 symlink로 공유해 두 runtime의 절차가 함께 갱신됨
- `_workspace`와 비-source 편집은 허용. 셸을 통한 간접 쓰기는 현재 보장 범위 밖
